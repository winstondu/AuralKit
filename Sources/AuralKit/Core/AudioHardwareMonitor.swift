...
internal actor AudioHardwareMonitor {
    ...
    private func setupNotifications() {
        ...
        let silenceSecondaryObserver = NotificationCenter.default.addObserver(
            ...
            Task {
                await self?.handleSilenceSecondaryAudioHint(notification)
            }
        }
    ...
    private func handleRouteChange(_ notification: Notification) async {
        ...
        let previousRoute = userInfo[AVAudioSession.routeChangePreviousRouteKey] as? AVAudioSessionRouteDescription
        let audioSession = AVAudioSession.sharedInstance()
    ...
    private func handleSilenceSecondaryAudioHint(_ notification: Notification) async {
        #if os(iOS) || os(tvOS)
        guard let userInfo = notification.userInfo,
              let typeValue = userInfo[AVAudioSession.silenceSecondaryAudioHintTypeKey] as? UInt,
